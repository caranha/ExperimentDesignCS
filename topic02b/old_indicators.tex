...
\section{Statistical Indicators}

\begin{frame}[t]{Section II: Statistical Indicators}

  In this section we talk more formally about how we define variables and models for our experiments, and about Statistical Indicators.

\end{frame}

\begin{frame}[t]{Data and "Experiment Data"}
  ...
  In this class, we are talking about "Experiment Data", which should be understood as "Data about the result of an experiment". {\bf For example}: How long did the experiment take? What is the success rate of my program?\bigskip
...
\begin{frame}{Population and Sample}
  ...
  \structure{Observation, Sample, and Population}.\bigskip

  \begin{itemize}
    \item {\bf Observation}: This is the result of one experiment: One student from the university, or one execution of the program.\medskip

    \item {\bf Sample}: This is a set of observations. When we repeat the experiment to observe the variation in the results, we collect a sample.\medskip

    \item {\bf Population}: This is the set of all possible experiment results. It can be a real set (all students of the university), or a theoretical set (all possible times for going from the dorm to class).
  \end{itemize}
  \bigskip

  The general goal of an experiment is {\bf "to learn something about the population, by examining the observations in the sample"}.
\end{frame}
...
\begin{frame}{Population, Model and Parameters}{What is a model?}
  \structure{A model is a description of the population}, describing the relationship between the conditions of the experiment and its possible results.\bigskip

  \begin{itemize}
    \item The balls pool has balls distributed {\bf uniformly}, so any sample that I take is a good representation of the population.
    \item If we measure the height of all students, the aggregate values can be represented by a {\bf normal distribution}.
    \item We model spread of a sickness in a city by dividing the inhabitants in {\bf susceptible}, {\bf infected} and {\bf Recovering} groups, and defining {\bf transition probabilities} between the groups.
  \end{itemize}\bigskip

  One goal of many experiments is to {\bf validate} or {\bf clarify} the details of a model.
\end{frame}

\begin{frame}{Population, Model and Parameters}{Example of model parameters}
  \begin{columns}[T]
    \column{0.5\textwidth}
      The analysis of experimental data can be used to estimate the parameters of a model.\bigskip

      The model is determined during the experiment design phase.\medskip

      Each parameter in the model has "true values" (values that reflect the population, $\theta$), and "estimated values" (values that are observed from the experiment, $\hat{\theta}$).\bigskip

      (Note that this setup assumes the model is correct, or a good approximation of reality!)
    \column{0.5\textwidth}
...
\begin{frame}[t]{CI on the Mean of a Normal Variable}
  {Known Variance Case}
...
For our coaxial cable example we have:
\begin{itemize}
  \item $\bar{x}$: Mean resistance: 48
  \item $z_{\alpha/2}$: 2.5 and 97.5 percentiles of the z distribution: 1.96, -1.96
  \item $\sigma$: assumed standard deviation: 2; $n$: Number of repetitions: 25
\end{itemize}\medskip

The result will be a confidence interval between 47.216 to 48.784
\end{frame}

\begin{frame}{CI with unknown variance}
For the more usual case with an unknown variance:

\begin{itemize}
  \item Replace the assumed standard deviation $\sigma$ with the calculated error from the experimental result, $s$.
  \item Replace the normal distribution with the student-t distribution.
\end{itemize}
